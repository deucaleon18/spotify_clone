...
const AddingToPlaylist = () => {
  ...
  const removeSongfromPlaylist = async (ID) => {
    ...
    const results = await fetch(
      `${url}playlist/${id}/tracks&songs=${ID}&request_method=delete&access_token=${access_token}`
    );
    const data = await results.json();
    console.log(data);
          ...
          {songloading ? null : (
            <div>
              <div className="playlist-area">
                ...
                <Box>
                  <button
                    className="add-songs-playlist-button"
                    style={{ height: "40px", cursor: "pointer" }}
                    onClick={() => {
                      showSearchSongsPopper();
                    }}
                  >
            ...
            <div className="search">
              <div className="search-area">
                <div className="bar">
                  ...
                  {loading ? //  (<div className="search-area">
                  null : (
                    // </div>)
                      ...
                      {show ? (
                        searchresult.map((searcher) => {
                          ...
                          const addtoThisPlaylist = async (ID) => {
                            ...
                            const access_token = localStorage.getItem("token");
                            const results = await fetch(
                              `${url}playlist/${id}/tracks&songs=${ID}&order=${ID}&request_method=post&access_token=${access_token}`
                            );
                            const data = await results.json();
                            console.log(data);
                            // window.location.reload();
                            if (data) {
                              alert(
                                "This song has been added.Please check your playlist in your library to play the song."
                              );
                            }
                          };
                          ...
                          return (
                            <Box display="flex" style={{ height: "8vh" }}>
                              ...
                              <Grid
                                ...
                                lg={12}
                                key={searcher.id}
                                style={{ alignItems: "center", height: "9vh" }}
                                onClick={() => {
                                  window.location.href = `/this/song/${id}`;
                                }}
                              >
